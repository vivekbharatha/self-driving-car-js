class Car {
  ...
  #assessDamage(roadBorders, traffic) {
    for (let v = 0; v < roadBorders.length; v++) {
      if (polysIntersect(this.polygon, roadBorders[v])) {
        return true;
      }
    }
    for (let v = 0; v < traffic.length; v++) {
      if (polysIntersect(this.polygon, traffic[v].polygon)) {
        return true;
      }
    }
    return false;
  ...
  draw(ctx, color, drawSensor = false) {
    ...
    for (let v = 1; v < this.polygon.length; v++) {
      ctx.lineTo(this.polygon[v].x, this.polygon[v].y);
    }
    ctx.fill();
